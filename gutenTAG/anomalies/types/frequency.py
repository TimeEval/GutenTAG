...
class AnomalyFrequency(BaseAnomaly):
    ...
    def generate(self, anomaly_protocol: AnomalyProtocol) -> AnomalyProtocol:
        if anomaly_protocol.base_oscillation_kind == BaseOscillationKind.Sinus:
            ...
            subsequence = sinus.generate_only_base(length, sinus.frequency * self.frequency_factor, freq_mod=sinus.freq_mod).reshape(-1)
            anomaly_protocol.subsequences.append(subsequence)
        elif anomaly_protocol.base_oscillation_kind == BaseOscillationKind.ECG:
            ecg = anomaly_protocol.base_oscillation
            subsequence = ecg.generate_only_base(frequency=ecg.frequency * self.frequency_factor).reshape(-1)[anomaly_protocol.start:anomaly_protocol.end]
            anomaly_protocol.subsequences.append(subsequence)
        else:
